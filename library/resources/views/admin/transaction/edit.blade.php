                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-8 row align-items-center">
                                <div class="input-group col-5 date">
                                    <input name="date_start" type="date" value="{{ $transaction->date_start }}"
                                        class="form-control">
                                </div>
                                <span class="col-auto"><i class="fa fa-minus"></i></span>
                                <div class="input-group col-5 date">
                                    <input name="date_end" type="date" value="{{ $transaction->date_end }}"
                                        class="form-control">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-8 select2-purple">
                                <select name="book_id[]" class="form-control select2" multiple="multiple"
                                    ...
                                    @foreach ($books as $book)
                                        <option {{ validate_valueIn($book->id, $select_books) == 1 ? 'selected' : '' }}
                                            value="{{ $book->id }}">{{ $book->title }}[{{ $book->qty }}]</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
